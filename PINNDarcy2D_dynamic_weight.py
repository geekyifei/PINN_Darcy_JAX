...
from jax import random, grad, vmap, jit, lax, tree_util
...
from tree_utils import tree_norm, tree_dot, tree_add
...
def FNN_with_batchnorm(layers, activation=jnp.tanh, epsilon=1e-5):
    ...
    def init(prng_key):
        def init_layer(key, d_in, d_out):
            ...
            W = glorot_stddev * random.normal(key1, (d_in, d_out))
            b = jnp.zeros(d_out)
            gamma = jnp.ones(d_out)  # Scale parameter for batch norm
            beta = jnp.zeros(d_out)  # Shift parameter for batch norm
    ...
    def forward(params, inputs, training=True, use_batchnorm=True):
        Z = inputs
        for W, b, gamma, beta in params[:-1]:
            outputs = jnp.dot(Z, W) + b
            if use_batchnorm and training:
                outputs = batch_norm(outputs, gamma, beta, epsilon)
            Z = activation(outputs)
...
class PinnDarcy2D:
    ...
    def __init__(self, layers_y, layers_h, params, dataset, key, **kwargs):
        ...
        #Note: this needs to be a tuple not list, to be hashable
        
        # weights dynamics
        self.adaptive_weight_bl_log = []
        self.adaptive_weight_br_log = []
        self.adaptive_weight_bt_log = []
        self.adaptive_weight_bb_log = []
        self.adaptive_weight_y_log = []
        self.adaptive_weight_h_log = []
        self.beta = 0.1
        ...
        if params['if_fourier_feature']:
            ...
            size_ff = 120
            scale_ff = 1.
            self.B = jax.random.normal(subkey, (size_ff, 2)) * scale_ff
        else:
            self.B = None
    ...
    #def get_grad_info(self):
        #norm_grad_r = tree_norm(grad_res)
        #dir_grad_bc = tree_dot(grad_bc, grad_res)/tree_norm(grad_res)*tree_norm(grad_bc)
    
    def grad_base_weight_update(self, grad_list):
        
        grad_r, grad_bl, grad_br, grad_bt, grad_bb = grad_list
        max_grad_r_list = tree_util.tree_map(jnp.max, 
                                             tree_util.tree_map(jnp.abs, 
                                                                grad_r
                                             ))
        mean_grad_bl_list = tree_util.tree_map(jnp.mean, 
                                             tree_util.tree_map(jnp.abs, 
                                                                grad_bl
                                             ))
        mean_grad_br_list = tree_util.tree_map(jnp.mean, 
                                             tree_util.tree_map(jnp.abs, 
                                                                grad_br
                                             ))
        mean_grad_bt_list = tree_util.tree_map(jnp.mean, 
                                             tree_util.tree_map(jnp.abs, 
                                                                grad_bt
                                             ))
        mean_grad_bb_list = tree_util.tree_map(jnp.mean, 
                                             tree_util.tree_map(jnp.abs, 
                                                                grad_bb
                                             ))
        max_grad_r = jnp.max(jnp.array(max_grad_r_list))
        mean_grad_bl = jnp.mean(jnp.array(mean_grad_bl_list)[0, :])
        mean_grad_br = jnp.mean(jnp.array(mean_grad_br_list)[0, :])
        mean_grad_bt = jnp.mean(jnp.array(mean_grad_bt_list)[0, :])
        mean_grad_bb = jnp.mean(jnp.array(mean_grad_bb_list)[0, :])
        self.lambda_bl = (max_grad_r / mean_grad_bl) * (1 - self.beta) \
                                 + self.beta * self.lambda_bl
        self.lambda_br = (max_grad_r / mean_grad_br) * (1 - self.beta) \
                                 + self.beta * self.lambda_br
        self.lambda_bt = (max_grad_r / mean_grad_bt) * (1 - self.beta) \
                                 + self.beta * self.lambda_bt 
        self.lambda_bb = (max_grad_r / mean_grad_bb) * (1 - self.beta) \
                                 + self.beta * self.lambda_bb                         
        return 
    
    def grad_base_weight_update_inverse(self, grad_list):
        
        grad_y, grad_h, grad_r, grad_bl, grad_br, grad_bt, grad_bb = grad_list
        max_grad_r_list = tree_util.tree_map(jnp.max, 
                                             tree_util.tree_map(jnp.abs, 
                                                                grad_r
                                             ))
        ...
        mean_grad_y_list = tree_util.tree_map(jnp.mean, 
                                             tree_util.tree_map(jnp.abs, 
                                                                grad_y
                                             ))
        mean_grad_h_list = tree_util.tree_map(jnp.mean, 
                                             tree_util.tree_map(jnp.abs, 
                                                                grad_h
                                             ))
        max_grad_r = jnp.max(jnp.array(max_grad_r_list))
        ...
        mean_grad_bt = jnp.mean(jnp.array(mean_grad_bt_list)[0, :])
        mean_grad_bb = jnp.mean(jnp.array(mean_grad_bb_list)[0, :])
        mean_grad_y = jnp.mean(jnp.array(mean_grad_y_list)[0, :])
        mean_grad_h = jnp.mean(jnp.array(mean_grad_h_list)[0, :])
        self.lambda_bl = (max_grad_r / mean_grad_bl) * (1 - self.beta) \
                                 + self.beta * self.lambda_bl
        self.lambda_br = (max_grad_r / mean_grad_br) * (1 - self.beta) \
                                 + self.beta * self.lambda_br
        ...
        self.lambda_bb = (max_grad_r / mean_grad_bb) * (1 - self.beta) \
                                 + self.beta * self.lambda_bb
        self.lambda_y = (max_grad_r / mean_grad_y) * (1 - self.beta) \
                                 + self.beta * self.lambda_y 
        self.lambda_h = (max_grad_r / mean_grad_h) * (1 - self.beta) \
                                 + self.beta * self.lambda_h
        return 
    ...
    @partial(jit, static_argnums=(0))
    def loss_bc_dar(self, params_y, params_h, boundary_idx, bc_type_idx, x_bc_dar, y_bc_dar):
    ...
    @partial(jit, static_argnums=(0,))
    def loss_bl(self, params_h, x_bl, y_bl):
        bl_pred = vmap(self.h_net, (None, 0, 0))(
                        params_h, x_bl[:,0], x_bl[:,1]
                        )
        # bl_pred = vmap(self.qx_net, (None, None, 0, 0))(
        #                 params_y, params_h, x_bl[:,0], x_bl[:,1]
        #                 )
        loss_bl = jnp.mean((bl_pred.flatten() - y_bl.flatten())**2)
        return loss_bl 
    
    @partial(jit, static_argnums=(0,))
    def loss_br(self, params_h, x_br, y_br):
        h_pred = vmap(self.h_net, (None, 0, 0))(
                        params_h, x_br[:,0], x_br[:,1]
                        )
        loss_br = jnp.mean((h_pred.flatten() - y_br.flatten())**2)
        return loss_br
    
    @partial(jit, static_argnums=(0,))
    def loss_bb(self, params_y, params_h, x_bb, y_bb):
        bb_pred = vmap(self.qy_net, (None, None, 0, 0))(
                        params_y, params_h, x_bb[:,0], x_bb[:,1]
                        )
        loss_bb = jnp.mean((bb_pred.flatten() - y_bb.flatten())**2)
        return loss_bb
    
    @partial(jit, static_argnums=(0,))
    def loss_bt(self, params_y, params_h, x_bt, y_bt):
        bt_pred = vmap(self.qy_net, (None, None, 0, 0))(
                        params_y, params_h, x_bt[:,0], x_bt[:,1]
                        )
        loss_bt = jnp.mean((bt_pred.flatten() - y_bt.flatten())**2)
        return loss_bt
    ...
    def loss_darcy_inverse(self, params):
        ...
        loss_bl = self.loss_bl(params_h, self.x_bl, self.y_bl)
        loss_br = self.loss_br(params_h, self.x_br, self.y_br)
        loss_bt = self.loss_bt(params_y, params_h, self.x_bt, self.y_bt)
        loss_bb = self.loss_bb(params_y, params_h, self.x_bb, self.y_bb)
        ...
        loss = self.lambda_y*loss_y + \
               ...
               #1e-4*(self.l2_reg(params_y) + self.l2_reg(params_h))
    ...
    def loss_darcy_forward(self, params_h):
        ...
        loss_bl = self.loss_bl(params_h, self.x_bl, self.y_bl)
        loss_br = self.loss_br(params_h, self.x_br, self.y_br)
        loss_bt = self.loss_bt(self.params_y, params_h, self.x_bt, self.y_bt)
        loss_bb = self.loss_bb(self.params_y, params_h, self.x_bb, self.y_bb)
    ...
    def train_y(self, num_epoch, num_print, y_test,
        ...
        if if_lbfgs == True:
            ...
            def step_lbfgs(carry):
                ...
                value, grad = value_and_grad_fn(params_y, state=opt_state_y)
                updates_y, opt_state_y = lbfgs_opt.update(grad, opt_state_y, params_y, 
            ...
            total_epoch = num_epoch + otu.tree_get(opt_state_y, 'count')
    ...
    def train_h(self, num_epoch, num_print, h_test,
        ...
        if if_lbfgs == True:
            ...
            def step_lbfgs(carry):
                ...
                value, grad = value_and_grad_fn(params_h, state=opt_state_h)
                updates_h, opt_state_h = lbfgs_opt.update(grad, opt_state_h, params_h, 
                                                          value=value, grad=grad, value_fn=self.loss_h
                                                          )
                params_h = optax.apply_updates(params_h, updates_h)
    ...
    def train_darcy_forward(self, num_epoch, num_print, if_full_batch, batch_size,
        ...
        if if_full_batch == True:
            for it in pbar:
                ...
                if it % 100 == 0:
                    grad_r = grad(self.loss_r, argnums=1)(self.params_y, self.params_h, self.x_r, self.y_r)
                    grad_bl = grad(self.loss_bl, argnums=0)(self.params_h, self.x_bl, self.y_bl)
                    grad_br = grad(self.loss_br, argnums=0)(self.params_h, self.x_br, self.y_br)
                    grad_bt = grad(self.loss_bt, argnums=1)(self.params_y, self.params_h, self.x_bt, self.y_bt)
                    grad_bb = grad(self.loss_bb, argnums=1)(self.params_y, self.params_h, self.x_bb, self.y_bb)
                    grad_list = (grad_r, grad_bl, grad_br, grad_bt, grad_bb)
                    self.grad_base_weight_update(grad_list)
                    self.adaptive_weight_bl_log.append(self.lambda_bl)
                    self.adaptive_weight_br_log.append(self.lambda_br)
                    self.adaptive_weight_bt_log.append(self.lambda_bt)
                    self.adaptive_weight_bb_log.append(self.lambda_bb)
        ...
        if if_lbfgs == True:
            ...
            def step_lbfgs(carry):
                ...
                value, grad = value_and_grad_fn(params_h, state=opt_state_h)
                updates_h, opt_state_h = lbfgs_opt.update(grad, opt_state_h, params_h, 
                                                          value=value, grad=grad, value_fn=self.loss_darcy_forward
                                                          )
                params_h = optax.apply_updates(params_h, updates_h)
                return (params_h, opt_state_h)
    ...
    def train_darcy_inverse(self, num_epoch, num_print, if_full_batch, batch_size,
        ...
        if if_full_batch == True:
            ...
            for it in pbar:
                ...
                if it % 100 == 0:
                    grad_r = grad(self.loss_r, argnums=1)(params['params_y'], params['params_h'], self.x_r, self.y_r)
                    grad_h = grad(self.loss_h, argnums=0)(self.params_h, self.x_h, self.y_h)
                    grad_y = grad(self.loss_y, argnums=0)(self.params_y, self.x_y, self.y_y)
                    grad_bl = grad(self.loss_bl, argnums=0)(self.params_h, self.x_bl, self.y_bl)
                    grad_br = grad(self.loss_br, argnums=0)(self.params_h, self.x_br, self.y_br)
                    grad_bt = grad(self.loss_bt, argnums=1)(params['params_y'], params['params_h'], self.x_bt, self.y_bt)
                    grad_bb = grad(self.loss_bb, argnums=1)(params['params_y'], params['params_h'], self.x_bb, self.y_bb)
                    grad_list = (grad_y, grad_h, grad_r, grad_bl, grad_br, grad_bt, grad_bb)
                    self.grad_base_weight_update_inverse(grad_list)
                    self.adaptive_weight_bl_log.append(self.lambda_bl)
                    self.adaptive_weight_br_log.append(self.lambda_br)
                    self.adaptive_weight_bt_log.append(self.lambda_bt)
                    self.adaptive_weight_bb_log.append(self.lambda_bb)
                    self.adaptive_weight_y_log.append(self.lambda_y)
                    self.adaptive_weight_h_log.append(self.lambda_h)
        ...
        if if_lbfgs == True:
            ...
            def step_lbfgs(carry):
                ...
                updates, opt_state = lbfgs_opt.update(grad, opt_state, params, 
                                                          value=value, grad=grad, value_fn=self.loss_darcy_inverse
                                                          )
                params = optax.apply_updates(params, updates)
            ...
            def stopping_criterion(carry):
                ...
                grad_l2_norm = otu.tree_l2_norm(grad)
                return (iter_num == 0) | ((iter_num < max_iter_lbfgs) & (grad_l2_norm >= rtol))
